    ...
    socket.on('new_bet', function(payload) {
        setTimeout(function(){
            ...
            if(user.uname == payload.uname){
                ...
                cell4.innerHTML = parseFloat(payload.wager/100).formatMoney(2,'.',',');

                cell5.innerHTML = parseFloat((payload.profit>=0?(payload.profit+payload.wager):payload.profit)/100).formatMoney(2,'.',',');
            ...
            cell3.innerHTML = ((parseFloat(payload.wager)+parseFloat(payload.profit))/parseFloat(payload.wager)).formatMoney(2, '.', ''))+"x";
            cell3.className = (parseFloat(payload.profit)>1?"table-result up":(parseFloat(payload.profit)<1?(parseFloat(payload.profit)==0?"table-result equal":"table-result down"):"table-result equal"));
