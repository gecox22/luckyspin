...
var hearthbeat = setInterval(function(){
    $.getJSON("https://api.moneypot.com/v1/auth?access_token="+user.token, function(json){
        if(Math.floor(json.user.balance)toFixed(2) != user.balance.toFixed(2)){
            if(Math.floor(json.user.balance) - user.balance > 0){
                $.notify({
                    icon: 'glyphicon glyphicon-warning-sign',
                    title: '<b>Balance update</b>:',
                    message: 'Received <b>'+parseFloat((Math.floor(json.user.balance) - user.balance)/100).formatMoney(2,'.',',')+'</b> bits!' 
                },{
                    type: 'success',
                    allow_dismiss: true,
                    placement: {
