    ...
    window.addEventListener('message', function(event) {
        if (event.origin === 'https://www.moneypot.com' && event.data === 'UPDATE_BALANCE') {
            $.getJSON("https://api.moneypot.com/v1/auth?access_token="+user.token, function(json){
                if(json.user.balance - user.balance != 0){
                    if(json.user.balance - user.balance > 0){
                        $.notify({
                            icon: 'glyphicon glyphicon-warning-sign',
                            title: '<b>Balance update</b>:',
                            message: 'Deposited <b>'+parseFloat((json.user.balance - user.balance)/100).formatMoney(2,'.',',')+'</b> bits!' 
                        },{
                            type: 'success',
                            allow_dismiss: true,
                            placement: {
                        ...
                        $.notify({
                            icon: 'glyphicon glyphicon-warning-sign',
                            title: '<b>Balance update</b>:',
                            message: 'Withdrawn <b>'+parseFloat((json.user.balance/100) - (user.balance/100)).formatMoney(2,'.',',')+'</b> bits!'
                        },{
                            type: 'warning',
                            allow_dismiss: true,
                            placement: {
                ...
                if(user.balance < 0.01) user.balance = 0;
