...
$(function(){
    ...
    $('#2x').click(function(){
        $('#wager-result').val(parseFloat((parseFloat($('#wager-result').val()) * 2).formatMoney(2,'.',',')));
    });
    $('#min').click(function(){
        $('#wager-result').val(parseFloat(((parseFloat($('.balance-result').val())>=1?1:0)).formatMoney(2,'.',',')));
    });
    $('#max').click(function(){
        $('#wager-result').val(parseFloat(parseFloat($('.balance-result').text()).formatMoney(2,'.',',')));
    });
});
...
function addNewChatMessage(data){
    ...
    chatMonitor.innerHTML += "<div class=\"chat_message "+rank+"\"><span class=\"time\">"+date.hours+":"+date.mins+"</span> <span class=\"username\">"+username+":</span> <span class=\"message\">"+urlify(message)+"</span></div>";
