...
totalbetsdone = 0;
startingbet = parseFloat(user.balance);
autobet();
...
window.alert('Error: Multiply on win and/or loss are not numbers!');
    ...
    $('#50p').click(function(){
        $('#wager-result').val(parseFloat((parseFloat(user.balance) * 0.5).toFixed(2)));
    });
    $('#2x').click(function(){
        $('#wager-result').val(parseFloat((parseFloat(user.balance) * 2).toFixed(2)));
    });
    $('#min').click(function(){
        $('#wager-result').val(parseFloat(((parseFloat(user.balance)>=1?1:0)).toFixed(2)));
		startingbet = user.balance;
    });
    $('#max').click(function(){
        $('#wager-result').val(parseFloat(user.balance));
		startingbet = user.balance;
    });
});
    ...
    getHash(function(){
        ...
        for(var i=0; i<wheels[wheelNumber].length; i++){
            payouts.push({
                ...
                value: (wheels[wheelNumber][i].value*(parseFloat(user.balance)))*100
            });
        }
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                ...
                wager: (parseFloat(user.balance)*100),
                payouts: payouts
            }),
            dataType: "json",
...
if (totallosses < startafterlosses && startafterlosses > 0) {
			...
			if (data.profit < 0 && multiplyonloss > 0){
			$('#wager-result').val(parseFloat((parseFloat(user.balance) * multiplyonloss/100).toFixed(2)));
			if(parseFloat(user.balance) > max_bet && max_bet > 0){
			...
			$('#wager-result').val(parseFloat((parseFloat(user.balance) * multiplyonwin/100).toFixed(2)));
			totallosses = 0;
			} else if (multiplyonwin == 0 && data.profit > 0) {
			$('#wager-result').val(parseFloat((parseFloat(startingbet).toFixed(2))));
			totallosses = 0;
