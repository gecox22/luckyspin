...
var hearthbeat = setInterval(function(){
    $.getJSON("https://api.moneypot.com/v1/auth?access_token="+user.token, function(json){
        if((json.user.balance/100).toFixed(2) != user.balance.toFixed(2)){
            if((json.user.balance/100) - user.balance > 0.01){
                $.notify({
                    icon: 'glyphicon glyphicon-warning-sign',
                    title: '<b>Balance update</b>:',
                    message: 'Received <b>'+parseFloat((json.user.balance/100) - user.balance).formatMoney(2,'.',',')+'</b> bits!' 
                },{
                    type: 'success',
                    allow_dismiss: true,
                    placement: {
