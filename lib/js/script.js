...
$(function(){
    ...
    socket.on('connect', function() {
        ...
        $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+(user.token!=null?user.token:undefined)+"&app_id="+app.id+"&limit=25&order_by=desc", function(json){
            if(json.length > 0){
                $.each(json.reverse(), function(index){
                    var payload = json[index];
                    if(user.uname == payload.uname){
                        ...
                        cell2.innerHTML = (payload.profit>0?parseFloat(parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager)).formatMoney(2, '.', ','))+"x":0+"x");
                        cell2.className = (payload.profit>1?"table-result up":(payload.profit<1?"table-result down":"table-result equal"));
                    ...
                    cell3.innerHTML = (payload.profit>0?parseFloat(parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager)).formatMoney(2, '.', ','))+"x":0+"x");
                    cell3.className = (payload.profit>1?"table-result up":(payload.profit<1?"table-result down":"table-result equal"));
    ...
    socket.on('new_bet', function(payload) {
        if(user.uname == payload.uname){
            ...
            cell2.innerHTML = (payload.profit>0?parseFloat(parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager)).formatMoney(2, '.', ','))+"x":(parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager))==1?1+"x":0+"x"));
            cell2.className = ((parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager))>1?"table-result up":(parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager))<1?"table-result down":"table-result equal"));
        ...
        cell3.innerHTML = (payload.profit>0?parseFloat(parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager)).formatMoney(2, '.', ','))+"x":(parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager))==1?1+"x":0+"x"));
        cell3.className = ((parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager))>1?"table-result up":(parseFloat(parseFloat(payload.profit)/parseFloat(payload.wager))<1?"table-result down":"table-result equal"));
    ...
    $(".login_button").click(function(){
        if(user.connected){
            ...
            window.location.href = "https://blog.moneypot.com/oauth/authorize?app_id=1269&response_type=token&state=Meh&redirect_uri=http://gecox22.github.io/luckyspin/";
        }
    });
...
function placeBet(wheelNumber){
    ...
    getHash(function(){
        ...
        $.ajax({
            ...
            var prizeNumber = 0;
            for(var i=0; i<wheels[wheelNumber].length; i++){
                if(outcome<Math.floor((Math.pow(2,32)/wheels[wheelNumber].length)*(i+1))){
                    prizeNumber = i;
                    break;
                }
            }
...
function SPIN(number){
    var $r = $('.wheel-active img').fortune(wheels[currentWheel].length);
    number = parseFloat(parseInt(number)+5.5);
